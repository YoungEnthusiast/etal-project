...
def post_list(request, tag_slug=None):
    ...
    return render(request, 'discover/posts.html', {'page': page,
                                                   'posts': posts,
                                                   'tag': tag})
...
class PostListView(ListView):
    ...
    paginate_by = 30
    template_name = 'discover/posts.html'

# def showPosts(request):
#     queryset = Post.published.all()
#     paginate_by = 30
#     paginator = Paginator(queryset, 30) # 3 posts in each page
#
#     return render(request, 'discover/posts.html', {'posts': queryset})
...
#     paginated_filtered_posts = Paginator(filtered_posts.qs, 1)
#     page_number = request.GET.get('page')
#     posts_page_obj = paginated_filtered_posts.get_page(page_number)
#     context['posts_page_obj'] = posts_page_obj
#     total_posts = filtered_posts.qs.count()
...
#
#     context['form'] = form
#     return render(request, 'discover/posts.html', context=context)
...
def post_detail(request, year, month, day, post):
    ...
    similar_posts = similar_posts.annotate(same_tags=Count('tags')).order_by('-same_tags',
                                                                             '-publish')[:4]
    return render(request, 'discover/post_detail.html', {'post': post,
                                                     'comments': comments,
